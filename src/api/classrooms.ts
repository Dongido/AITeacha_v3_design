...
export const getPretestQuestions = async (classroomId: number) => {
  const { data } = await apiClient.get(
    `/classroom/get/pretest/questions/${classroomId}`
  );
  return data;
};

export const submitPretestAnswers = async (
  classroomId: number,
  answers: any[]
) => {
  const { data } = await apiClient.post(
    `/classroom/submit/student/pretest`,
    {
      classroom_id: classroomId,
    ...
    {
      headers: {
        "x-api-key": "YOUR_API_KEY_HERE", // ✅ same as used for getPretestQuestions
      },
    }
  );
  return data;

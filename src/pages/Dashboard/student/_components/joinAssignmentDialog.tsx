...
import { joinAssignment } from "../../../../api/studentassignment";
...
const JoinAssignment = forwardRef(
  ({ classId, onSuccess = () => {} }: IProps, ref) => {
    ...
    const onSubmit = async (data: z.infer<typeof formSchema>) => {
      ...
      try {
        const response = await joinAssignment(data.code);
        if (response.status === "success") {
          setToastMessage("Successfully joined the assignment group!");
        ...
        <>
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogContent className="max-w-[450px] text-gray-800">
              <DialogHeader>
                <DialogTitle>Join Assignment Group</DialogTitle>
              </DialogHeader>
              <DialogDescription>
                Enter the class code to join the assignment.
              </DialogDescription>
              ...
              <FormProvider {...formMethods}>
                <form onSubmit={handleSubmit(onSubmit)}>
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Assignment Code</FormLabel>
                        <FormControl>
                          <Input
                            type="text"

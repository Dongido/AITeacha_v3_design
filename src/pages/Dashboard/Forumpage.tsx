...
const Forumpage = () => {
  ...
  if (error === "Failed to fetch premium users") {
    return (
      <div>
        {userDetails && isEmailVerified === 1 && (
          <div
  ...
  return (
    <div className="min-h-screenp p-4 md:p-[30px] ">
      ...
      {!loading && users?.length > 0 && (
        <div className="grid  border gap-6 my-10">
          {users.map((user: any) => (
            <>
              <div
                // key={user.user_id}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col gap-1 text-start">
                    <h5 className="font-semibold text-base m-0">
                      {user.organization?.trim()
                        ? user.organization
                        : `${user.firstname} ${user.lastname}`}
                    </h5>
                    <p className="text-gray-600 text-sm m-0">
                      This forum gives you access to {user.organization} team
                      discussions.
                    </p>
                  </div>
                </div>
                ...
                <Link
                  ...
                  className="font-medium text-sm flex text-purple-700 hover:text-purple-900 cursor-po items-center gap-2 transition"
                >
                  View Topics

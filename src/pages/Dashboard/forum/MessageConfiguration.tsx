...
//       <h1 className="md:text-3xl text-2xl text-center mb-0 font-bold text-gray-800">Message Configuration</h1>
//       <p className="text-gray-600 text-base text-center mt-0 mb-10">
...
//               <p className="text-lg m-0 font-medium text-gray-800">{item.title}</p>
...
const MessageConfiguration = () => {
  ...
  useEffect(() => {
    if (messageSettingsId && messageSettingsId.user_id === userId) {
      setForm({
        enable_teacher_student_chat:
          !!messageSettingsId.enable_teacher_student_chat,
        enable_student_teacher: !!messageSettingsId.enable_student_teacher,
        enable_teacher_teacher: !!messageSettingsId.enable_teacher_teacher,
        enable_student_student: !!messageSettingsId.enable_student_student,
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (messageSettingsId && messageSettingsId.user_id === userId) {
        await dispatch(
          updatemessageSetting({ ...payload, id: messageSettingsId.id })
        ).unwrap();
        alert("Settings updated successfully!");
      } else {
        await dispatch(createmessageSetting(payload)).unwrap();
    ...
    <div className="min-h-screen max-w-5xl mx-auto px-2 md:px-6 py-10">
      ...
      <div className="max-w-2xl mx-auto">
        {[
          ...
          <div
            key={index}
            className="flex justify-between items-center bg-white p-5 border-b border-gray-100"
          >
            <div className="flex-1 pr-4">
              <p className="text-lg m-0 font-medium text-gray-800">
                {item.title}
              </p>
              <p className="text-sm m-0 text-gray-500">{item.desc}</p>
            </div>
            ...
            {/* ✅ Custom Toggle */}
            <button
              onClick={() => handleToggle(item.key as keyof typeof form)}
            ...
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-deep-purple-900 transition-transform duration-300 ${
                  form[item.key as keyof typeof form]
                    ? "translate-x-6"
                    : "translate-x-1"
                }`}

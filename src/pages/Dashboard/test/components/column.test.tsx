...
export const testColumns = [
  ...
  testColumnHelper.accessor("school_name", {
    header: ({ column }) => (
      <div className="hidden md:table-cell">
        <Header title="School Name" column={column} />
      </div>
    ),
    sortingFn: "text",
    cell: (info) => {
      ...
      return (
        <Link
          to={redirectPath}
          className="hidden md:inline capitalize text-primary whitespace-nowrap"
        >
          {info.getValue() || "N/A"}
        </Link>
      );
  ...
  testColumnHelper.accessor("join_url", {
    ...
    cell: (info) => {
      const navigate = useNavigate();
      const test = info.row.original;
      const examinationId = test.examination_id;
      const status = test.status;

      const deleteDialogRef = useRef<{ openDialog: () => void }>(null);
      const activateDeactivateDialogRef = useRef<{ openDialog: () => void }>(null);

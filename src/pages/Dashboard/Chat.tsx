...
// // update on the zyra page
// const [sidebarCollapsed, setSidebarCollapsed] = useState(false);
// const [chatHistory, setChatHistory] = useState<{id: string, title: string}[]>([]);
// const [activeChatId, setActiveChatId] = useState<string | null>(null);

// const startNewChat = () => {
//   setActiveChatId(null); // Clear messages for new chat
// };

// const selectChat = (id: string) => {
//   setActiveChatId(id); // Load chat messages from API
// };
...
// //     fetchChatHistory(activeChatId).then((msgs) => setMessages(msgs));
// //   } else {
...
//   };
//   return (
//     <div className="min-h-screen border-4 border-green-700 z-[9999] fixed inset-0  bg-[linear-gradient(to_right,#6200EE1A_10%,#F133E11A_90%)] overflow-auto flex flex-col ">

//       {/* Sidebar */}
//   <div
//     className={`flex flex-col bg-white min-h-screen border-r transition-all duration-300 ${
...
//           className="p-2 m-2 bg-purple-500 text-white rounded"
//         >
//           + New Chat
//         </button>
...
//  {/* Chat Content */}
//   <div className="flex-1 flex flex-col relative overflow-auto bg-green-400">
//     <button
//       onClick={closeChat}
...
//     {/* Input area, buttons, file upload */}
//   </div>
// </div>
...
import { BsLayoutSidebar, BsLayoutSidebarInsetReverse } from "react-icons/bs";
...
const Chat = () => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex overflow-auto  bg-[linear-gradient(to_right,#6200EE1A_10%,#F133E11A_90%)] min-h-screen">
      {/* Sidebar */}
      <div
        className={`flex py-[30px] ${
      ...
      >
        <div className="px-1 mb-2">
          <NavLink
            ...
            className={`flex items-center   rounded-md font-semibold transition-all bg-[#EFE6FD] text-sm ${
              !sidebarCollapsed ? "px-3 py-2 gap-3" : "p-1"
            }    ${
            ...
            <div className="relative flex items-center gap-3">
              ...
              {!sidebarCollapsed && (
                <div className="flex flex-col">
                  <span className="font-semibold text-[16px] text-[#2A2929]">
                    Zyra
                  </span>
        ...
        <div>
          <div className="px-1 mb-2 mt-[40px]">
            <button
              // onClick={() => isMobile && setOpenSidenav(dispatch, false)}
              className={`flex items-center  font-semibold transition-all text-sm ${
                !sidebarCollapsed ? "px-3 py-2 gap-3" : "p-1"
              }`}
            >
              <div className="relative flex items-center gap-3">
                <FaRegEdit size={20} />
                {!sidebarCollapsed && (
                  <p className="font-semibold text-[16px] m-0 text-[#2A2929]">
                    New Chat
                  </p>
                )}
              </div>
      ...
      {/* Chat Content */}
      <div className="flex-1 bg-[linear-gradient(to_right,#6200EE1A,#F133E11A_90%)] flex flex-col relative overflow-auto p-4">
        <div className="flex items-center justify-between ">
          <button
            onClick={() => setSidebarCollapsed(!sidebarCollapsed)}
            className="p-2"
          >
            {sidebarCollapsed ? <BsLayoutSidebar /> : <BsLayoutSidebar />}
          </button>
        ...
        {/* <div className="mt-4 flex items-center gap-2">
          <TextArea value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Type your message..." className="flex-1 p-2 rounded-full border" />
          <button onClick={toggleRecording} className={`p-2 rounded-full ${isRecording ? "bg-red-500 text-white" : "bg-gray-200"}`}><FiMic /></button>
          <button onClick={handleSend} className="p-2 bg-primary text-white rounded-full"><FiSend /></button>
        </div> */}
          ...
          <div className="flex-grow overflow-y-auto  rounded-lg   p-4">
            {messages.length === 0 ? (
              <div className="text-center text-gray-800 italic">
                Hi👋, welcome to{" "}
                <span className="font-bold">Zyra (Edubot)</span>!
                <div className="mt-4 text-sm lg:text-lg max-w-xl mx-auto text-gray-600">
                  Zyra is here to assist you with educational tasks, provide
                  resources, answer your questions, and help you learn better.
                </div>
            ...
            <div className="w-full  relative flex items-center  rounded-lg">
              ...
              <Dialog>
                ...
                <DialogContent className="p-6 rounded-lg shadow-lg">
                  ...
                  <p className="text-md text-gray-500 mt-1">
                    Enabling this option will use the uploaded files in your
                    archive for better response.
                  </p>

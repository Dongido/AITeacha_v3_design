...
//     // Simulate AI or API suggestion logic
//     setTimeout(() => {
//       const base = className.toLowerCase();

//       // Example logic — you can replace with API call if needed
//       const generated = [
//         `An introduction to ${base} fundamentals for beginners.`,
//         `A comprehensive guide to mastering ${base}.`,
//         `Hands-on ${base} lessons with practical examples.`,
//         `Explore key concepts in ${base} to build strong foundational skills.`,
//       ];

//       setSuggestedDescriptions(generated);
...
//     console.log("submitting classroom data")
...
//       user_id: userId,
//       description,
//       youtube_links : ["https://www.youtube.com/watch?v=KFzrkgk-9YA"],
//       grade: values.grade,
//       status: "active",
...
//     if (
//       currentStep === 3 &&
//       (!uploads || Object.values(uploads).flat().length === 0)
//     ) {
//       showToast("No files uploaded yet", "default");
//     }
...
//                             Start typing your classroom name to get suggestions.
//                           </p>
//                         )}
//                       </div>
//                     )}
...
//             setUploads((prev) => ({
//               ...prev,
//               youtube: prev.youtube.filter((_, idx) => idx !== i),
//             }))
//           }
//           className="text-red-500 hover:text-red-700 text-xs font-semibold"
//         >
...
const CreateOrEditClassroom = ({ isEdit }: { isEdit?: boolean }) => {
  ...
  const handleRemoveSelectedOutline = (
    ...
    event.stopPropagation(); // ✅ prevents triggering any step navigation or form submission
    ...
    setSelectedOutlines((prevSelected) => {
      ...
      if (updatedSelectedOutlines.length === 0) {
        localStorage.removeItem(CLASSROOM_DRAFT_KEY);
        setParsedOutlines([]);
      } else {
        localStorage.setItem(
          CLASSROOM_DRAFT_KEY,
          JSON.stringify(updatedSelectedOutlines)
        );
      }
  ...
  useEffect(() => {
    ...
    // Simulate AI logic (replace with backend later)
    const timer = setTimeout(() => {
      const base = description.toLowerCase().split(" ").slice(0, 3).join(" "); // derive topic
      const generated = [
        `A comprehensive overview of ${base}, focusing on hands-on learning and interactive sessions.`,
        `Learn ${base} through engaging examples and practical exercises designed for beginners.`,
        `This course simplifies ${base} concepts to help students grasp fundamentals with ease.`,
        `An engaging and structured approach to ${base}, with real-world applications and projects.`,
      ];
      setSuggestedDescriptions(generated);
      setIsLoadingSuggestions(false);
    }, 900);
          ...
          <div className="max-w-3xl mx-auto">
            ...
            {currentStep === 2 && (
              <section className="max-w-3xl mx-auto">
                <div className="bg-white rounded-[32px] shadow-sm p-8 mb-8">
                  {/* Description */}
                  <div className="bg-white rounded-[32px] shadow-sm mb-8 max-w-3xl mx-auto">
                    ...
                    <textarea
                      value={description}
                      onChange={(e) => setDescription(e.target.value)}
                      placeholder="E.g. This course offers an introduction to web design concepts, tools & techniques, tailored for beginners..."
                      rows={5}
                      className="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm text-gray-700 focus:ring-2 focus:ring-[#6B46FF] resize-none"
                    />

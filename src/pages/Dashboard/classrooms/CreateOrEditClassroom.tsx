...
const CreateOrEditClassroom = ({ isEdit }: { isEdit?: boolean }) => {
  ...
  const goNext = async () => {
    if (currentStep === 1) {
      ...
      // 🔥 NEW: Fetch AI Suggestions here
      try {
        setIsLoadingSuggestions(true);
        const stepOneData = getValues();
        const topics = await suggestClassroomTopics(
          stepOneData.name,
          stepOneData.grade
        );
        ...
        showToast("Failed to load topic suggestions.", "destructive");
      } finally {
        setIsLoadingSuggestions(false);
      }
                        ...
                        <div className="mt-4">
                          ...
                          <div className="flex flex-wrap gap-2 mx-auto">
                            {suggestedDescriptions.map((desc, index) => (
                              <div
                                key={index}
                                className="p-2 border rounded-md bg-gray-100 text-sm hover:bg-[#EFE6FD] cursor-pointer"

...
import { useNavigate, useParams, Link } from "react-router-dom";
import DeleteClassroomDialog from "./components/DeleteClassroomDialogue";
import ActivateDeactivateDialog from "./components/ActivateDeactivateDialog"; // ✅ added
import { useDispatch, useSelector } from "react-redux";
import { fetchClassroomByIdThunk } from "../../../store/slices/classroomSlice";
import { RootState, AppDispatch } from "../../../store";
import { CheckIcon } from "@heroicons/react/24/solid";
...
const ClassroomDetail = () => {
  ...
  const dispatch = useDispatch<AppDispatch>();
  const deleteDialogRef = useRef<any>(null);
  const activateDialogRef = useRef<{ openDialog: () => void }>(null); // ✅ added
  ...
  const openDeleteDialog = () => deleteDialogRef.current?.openDialog();
  const openActivateDialog = () => activateDialogRef.current?.openDialog(); // ✅ added
  ...
  const status = classroom?.status?.toLowerCase();
    ...
    <div className="mt-4 mb-10">
      ...
      <div className="flex gap-2 justify-between">
        <div className="flex items-center gap-4">
          <p
            className={`text-sm mt-2 border rounded-full px-6 py-2 ${
              status === "active"
                ? "text-[#0DBA86] bg-[#E7F8F3] border-[#0DBA86]"
                : "text-[#E67E22] bg-[#FEF3E7] border-[#E67E22]"
            }`}
          >
            <strong>Status:</strong> {classroom?.status}
          </p>
      ...
      <ActivateDeactivateDialog
        ...
        classroomId={Number(id)}
        status={classroom?.status as string}
        onSuccess={() => dispatch(fetchClassroomByIdThunk(Number(id)))}
      />
    </div>
  );
};
